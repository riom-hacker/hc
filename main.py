...
async def generate_fampay_qr(user_id: int, amount: float, upi_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # The SDK is synchronous, so run it off the aiogram event loop.
...
async def run_payment_verification(user_id: int, order_id: str, reply_target: Any) -> None:
    ...
    if result.get("status") == "success":
        ...
        if isinstance(reply_target, CallbackQuery):
            try: await reply_target.message.edit_caption(success_msg, reply_markup=back_kb(), parse_mode='HTML')
            except Exception: await reply_target.message.edit_text(success_msg, reply_markup=back_kb(), parse_mode='HTML')
                ...
                async with session.get(qr_url, timeout=aiohttp.ClientTimeout(total=10), allow_redirects=True) as resp:
                    if resp.status == 200:
                        ...
                        if img_bytes:
                            ...
                            await bot.send_photo(message_obj.chat.id, BufferedInputFile(img_bytes, filename="qr.png"), caption=caption, reply_markup=kb, parse_mode='HTML')
                            qr_sent = True
        except Exception as e:
