...
def migrate_categories() -> None:
    ...
    # Keep admin-edited UI text. Only seed missing values; never overwrite
    # text that the admin has already customized from the Admin Panel.
    for _ui_key, _ui_default in UI_TEXTS.items():
        if not get_setting(f"ui_{_ui_key}", ""):
            set_setting(f"ui_{_ui_key}", _ui_default)
    logger.info("UI emojis updated; existing admin-customized UI texts preserved.")
...
def admin_kb() -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="📝 Welcome Text", callback_data="admin_edit_welcome_text", style="primary")
        ],
        [
...
async def auto_verify_task() -> None:
    """Poll pending FamGateway transactions every 1 second for up to 5 minutes."""
    while True:
...
async def generate_fampay_order(user_id: int, inr_amount: float, message_obj: Message) -> None:
    ...
    caption = (
        ...
        f"4️⃣ Auto-verify checks every 1 second for up to 5 minutes!"
    )
    ...
    if qr_url:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(qr_url, timeout=aiohttp.ClientTimeout(total=5), allow_redirects=True) as resp:
                    if resp.status == 200:
...
async def view_shop_panels(call: CallbackQuery):
    ...
    for cat_index, cat in enumerate(FIXED_CATEGORIES):
        emoji_id = get_category_emoji(cat)
        # Use a tiny numeric callback token instead of category text. This
        # avoids callback parsing/length issues completely.
        kb.inline_keyboard.append([
            InlineKeyboardButton(
                text=cat,
                callback_data=f"catid_{cat_index}",
...
async def view_panel_names(call: CallbackQuery):
    # Resolve category by its numeric button ID. This is safer than putting
    # category text into callback_data and prevents first-click/second-click
    # behaviour caused by malformed or mismatched callback values.
    try:
        cat_index = int(call.data.split("catid_", 1)[1])
        category = FIXED_CATEGORIES[cat_index]
    except (ValueError, IndexError):
        return await call.answer("❌ Invalid category selection.", show_alert=True)

    # Always normalize category matching (trim + case-insensitive). LIKE was
    # also unnecessarily treating '%' and '_' in category text as wildcards.
    category_sql = "TRIM(category) = TRIM(?) COLLATE NOCASE"

    panel_names = db_query(
        f"SELECT DISTINCT TRIM(panel_name) FROM products "
        f"WHERE {category_sql} AND is_active=1 AND TRIM(COALESCE(panel_name, '')) != '' "
        f"ORDER BY TRIM(panel_name)",
        (category,),
        fetchall=True
    ) or []
    ...
    if not panel_names:
        prods = db_query(
            f"SELECT id, name, price_inr, stock, reseller_price, validity, device_limit "
            f"FROM products WHERE {category_sql} AND is_active=1 ORDER BY id",
            (category,),
            fetchall=True
        ) or []
        if not prods:
            # Do not make the user press the same button again. Close the
            # callback immediately and show a normal message with a BACK button.
            await call.answer()
            try:
                await call.message.edit_text(
                    f"⚠️ <b>No active products found in {html.escape(category)}.</b>\n\n"
                    "Please ask the admin to check the product category/import.",
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    text = f"{get_emoji('product_store')} <b><u>{html.escape(category.upper())} PANELS</u></b>\n━━━━━━━━━━━━━━━━━━\n\n{get_emoji('point_down')} <b>Choose a panel name:</b>"
    for pn in panel_names:
        panel = (pn[0] or '').strip()
        if not panel:
            continue
        emoji_id = get_panel_emoji(panel) or get_emoji_icon("product_store")
        # Use a real product id as the callback token. This keeps callback_data
        # short and prevents long panel names/underscores from breaking the
        # selection when Telegram sends the callback.
        sample = db_query(
            f"SELECT id FROM products WHERE {category_sql} AND TRIM(panel_name) = TRIM(?) COLLATE NOCASE AND is_active=1 ORDER BY id LIMIT 1",
            (category, panel),
            fetchone=True
        )
        if not sample:
        ...
        kb.inline_keyboard.append([
            InlineKeyboardButton(
                text=panel,
                callback_data=f"pnlid_{sample[0]}",
                icon_custom_emoji_id=emoji_id,
                style="primary"
            )
        ])
...
@dp.callback_query(F.data.startswith("pnlid_"))
async def view_products_for_panel_id(call: CallbackQuery):
    try:
        prod_id = int(call.data.split("pnlid_", 1)[1])
    except (ValueError, IndexError):
    ...
    if not prod:
        return await call.answer("❌ Panel is no longer available.", show_alert=True)

    category, panel_name = (prod[0] or "").strip(), (prod[1] or "").strip()
    prods = db_query(
        "SELECT id, name, price_inr, stock, reseller_price, validity, device_limit "
        "FROM products WHERE TRIM(category) = TRIM(?) COLLATE NOCASE "
        "AND TRIM(panel_name) = TRIM(?) COLLATE NOCASE AND is_active=1 ORDER BY id",
        (category, panel_name),
        fetchall=True
    ) or []
    if not prods:
        return await call.answer("❌ No products found for this panel.", show_alert=True)
    # show_products_for_panel() handles the callback answer itself. Do not
    # answer the same Telegram callback a second time.
    await show_products_for_panel(call, prods, f"{category} - {panel_name}")


# Legacy pnl_ handler kept only for old messages/buttons. New buttons use
# pnlid_<product_id>, so normal navigation never depends on panel text parsing.
...
async def view_products_for_panel(call: CallbackQuery):
    ...
    category = urllib.parse.unquote(parts[0]).strip()
    panel_name = urllib.parse.unquote(parts[1]).strip()
    category = next((c for c in FIXED_CATEGORIES if c.lower() == category.lower()), category)
...
async def process_buy(call: CallbackQuery):
    ...
    if qr_url:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(qr_url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                    if resp.status == 200:
                        img_bytes = await resp.read()
...
async def admin_edit_welcome_text_start(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    current_text = get_ui_text("start_menu")
    await call.message.edit_text(
        "📝 <b>EDIT WELCOME TEXT</b>\n\n"
        "Send the complete welcome/start-menu text you want users to see.\n"
        "You can use multiple lines and HTML tags such as <b>bold</b>.\n\n"
        "<b>Current text:</b>\n" + current_text,
...
@dp.message(AdminStates.edit_welcome_text)
async def admin_save_welcome_text(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    new_text = (m.text or "").strip()
    if not new_text:
        await m.answer("❌ Welcome text cannot be empty. Send the complete text again.", reply_markup=admin_kb(), parse_mode='HTML')
        return
    db_query("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", ("ui_start_menu", new_text))
    await state.clear()
    await m.answer("✅ <b>Welcome text updated successfully!</b>\n\nThe new text will be shown from the next Start/Main Menu display.", reply_markup=admin_kb(), parse_mode='HTML')
