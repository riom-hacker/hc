...
async def auto_verify_task() -> None:
    ...
    while True:
        ...
        for order_id, user_id, amount, ts, expires_at in pending_txns:
            ...
            # Match the supplied FamGateway reference exactly in behavior:
            # delete the old QR message first, then send instant confirmation
            # containing the order ID, bank UTR and paid amount.
            ...
            try:
                await bot.send_message(
                    ...
                    f"Bank UTR: <code>{html.escape(str(utr))}</code>\n"
                    f"Amount: {fmt_curr(amount_received)}\n"
...
async def generate_fampay_order(user_id: int, inr_amount: float, message_obj: Message) -> None:
    ...
    if not qr_sent:
        try:
            # Keep the message ID even when Telegram cannot accept the QR as a
            # photo. This lets the 3-second verifier remove the payment message
            # after the order is paid, just like the reference implementation.
            await message_obj.edit_text(caption, reply_markup=kb, parse_mode='HTML')
            db_query(
                "UPDATE transactions SET qr_message_id=?, qr_chat_id=? WHERE order_id=?",
                (message_obj.message_id, message_obj.chat.id, order_id)
            )
        except Exception:
            try:
                fallback_msg = await bot.send_message(
                    message_obj.chat.id, caption, reply_markup=kb, parse_mode='HTML'
                )
                db_query(
                    "UPDATE transactions SET qr_message_id=?, qr_chat_id=? WHERE order_id=?",
                    (fallback_msg.message_id, message_obj.chat.id, order_id)
                )
            except Exception as e:
                logger.error("FamGateway fallback payment message failed: %s", e)
...
async def process_buy(call: CallbackQuery):
    ...
    if not qr_sent:
        try:
            # Store the fallback message ID so automatic verification can still
            # clean it up immediately after payment confirmation.
            await progress_message.edit_text(caption, reply_markup=kb, parse_mode='HTML')
            db_query(
                "UPDATE product_orders SET qr_message_id=?, qr_chat_id=? WHERE order_id=?",
                (progress_message.message_id, progress_message.chat.id, order_id)
            )
        except Exception:
            try:
                fallback_msg = await bot.send_message(
                    call.message.chat.id, caption, reply_markup=kb, parse_mode='HTML'
                )
                db_query(
                    "UPDATE product_orders SET qr_message_id=?, qr_chat_id=? WHERE order_id=?",
                    (fallback_msg.message_id, call.message.chat.id, order_id)
                )
            except Exception as e:
                logger.error("Product fallback payment message failed: %s", e)
